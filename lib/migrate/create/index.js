...
const modelDefinedFiles = {};

module.exports = async (dbConfig, entityPath, modelDefinedPath) => {
    ...
    if (stats.isFile()) {
        await create(entityPath, sequelize);
        if (modelDefinedPath) {
            await createModelDefined(modelDefinedPath, entityPath);
        }
    } else {
        const entityFiles = await fs.readdir(entityPath);
        for (const file of entityFiles) {
            const stats = await fs.stat(path.join(entityPath, file));
            if (!stats.isFile()) {
                console.log(`skip directory: ${path.join(entityPath, file)}`);
                continue;
            }
            await create(path.join(entityPath, file), sequelize);
            if (modelDefinedPath) {
                await createModelDefined(path.join(modelDefinedPath, path.basename(entityPath)), path.join(entityPath, file));
            }
        }
    }
    sequelize.close();
    return {migrateInfo, modelDefinedFiles};
};

async function create(entityFile, sequelize) {
...
async function createModelDefined(modelDefinedPath, entityFile) {
    if (!(await fs.exists(modelDefinedPath))) {
        await fs.mkdirp(modelDefinedPath);
    }
    const entity = require(entityFile);
    const attributesOut = generateExecute.outputAttributes(attributesToSequelizeOrm(entity.columns));
    modelDefinedFiles[path.join(modelDefinedPath, `${path.parse(entityFile).name}${path.parse(entityFile).ext}`)] = {
